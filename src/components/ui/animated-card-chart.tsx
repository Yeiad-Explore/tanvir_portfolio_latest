...
export function AnimatedCard({ className, ...props }: CardProps) {
  return (
    <div
      ...
      className={cn(
        "group/animated-card relative w-full overflow-hidden rounded-xl border border-gray-700 bg-gray-900/50 backdrop-blur-sm shadow-lg",
        className
      )}
      {...props}
    />
...
export function CardBody({ className, ...props }: CardProps) {
  return (
    <div
      role="group"
      className={cn(
        "flex flex-col space-y-1.5 border-t border-gray-700 p-4",
        className
      )}
      {...props}
    />
...
export function CardVisual({ className, ...props }: CardProps) {
  return (
    <div
      className={cn(
        "h-[180px] w-full overflow-hidden flex justify-center items-center",
        className
      )}
      {...props}
    />
...
export function Visual3({
  ...
  return (
    <>
      ...
      <div className="relative h-[180px] w-full max-w-[356px] mx-auto overflow-hidden rounded-t-lg bg-gradient-to-br from-gray-800/30 to-gray-900/50 backdrop-blur-sm">
        <Layer4
          color={mainColor}
...
const Layer1: React.FC<LayerProps> = ({ color, secondaryColor }) => {
  return (
    ...
    >
      <div className="flex shrink-0 items-center rounded-full border border-gray-600 bg-gray-800/40 px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0">
        <div className="h-1.5 w-1.5 rounded-full bg-[var(--color)]" />
        <span className="ml-1 text-[10px] text-white">+15,2%</span>
      </div>
      <div className="flex shrink-0 items-center rounded-full border border-gray-600 bg-gray-800/40 px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0">
        <div className="h-1.5 w-1.5 rounded-full bg-[var(--secondary-color)]" />
        <span className="ml-1 text-[10px] text-white">+18,7%</span>
      </div>
    </div>
  );
};
  ...
  return (
    ...
    >
      <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[7] flex w-full translate-y-full items-start justify-center bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-0">
        <div className="ease-[cubic-bezier(0.6, 0, 1)] rounded-md border border-gray-600 bg-gray-800/40 p-1.5 opacity-0 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-100">
          <div className="flex items-center gap-2">
            <div className="h-2 w-2 shrink-0 rounded-full bg-[var(--color)]" />
            <p className="text-xs text-white">Stats</p>
          </div>
...
const Layer4: React.FC<LayerProps> = ({ color, secondaryColor, hovered }) => {
  ...
  return (
    <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[8] flex h-[180px] w-full items-center justify-center text-white/10 transition-transform duration-500 group-hover/animated-card:scale-150">
      <svg
        width="356"
        height="180"
        xmlns="http://www.w3.org/2000/svg"
